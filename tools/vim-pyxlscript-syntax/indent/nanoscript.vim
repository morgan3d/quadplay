...
" Nanoscript indent file based on python
" Language:         Nanoscript
...
setlocal nolisp
setlocal autoindent
setlocal indentexpr=GetNanoscriptIndent(v:lnum)
setlocal indentkeys=!^F,o,O,<:>,0),0],0},=elif,=except

" TODO: check required patch for timeout argument, likely lower than 7.3.429 though.
if !exists('g:nanoscript_indent_searchpair_timeout')
    if has('patch-8.0.1483')
        let g:nanoscript_indent_searchpair_timeout = 150
    else
        let g:nanoscript_indent_searchpair_timeout = 0
    endif
endif
...
function! s:find_opening_paren(lnum, col)
    ...
    let nearest = [0, 0]
    let timeout = g:nanoscript_indent_searchpair_timeout
...
function! s:indent_like_opening_paren(lnum)
    ...
    let hang_closing = get(b:, 'nanoscript_indent_hang_closing',
                \ get(g:, 'nanoscript_indent_hang_closing', 0))
...
" Is the syntax at lnum (and optionally cnum) a nanoscript string?
function! s:is_nanoscript_string(lnum, ...)
    ...
    for cnum in cols
        if match(map(synstack(a:lnum, cnum),
                    \ "synIDattr(v:val, 'name')"), 'nanoscript\S*String') == -1
            return 0
        end
    endfor
    return 1
...
function! GetNanoscriptIndent(lnum)
    ...
    if s:is_nanoscript_string(a:lnum-1, max([1, len(prevline)]))
                \ && (s:is_nanoscript_string(a:lnum, 1)
                \     || match(line, '^\%("""\|''''''\)') != -1)
        ...
        if match_quotes != -1
            ...
            let pairpos = searchpairpos(quotes, '', quotes, 'bW', '', 0, g:nanoscript_indent_searchpair_timeout)
            if pairpos[0] != 0
        ...
        if s:is_nanoscript_string(a:lnum-1)
        ...
        let indent_multi = get(b:, 'nanoscript_indent_multiline_string',
                    \ get(g:, 'nanoscript_indent_multiline_string', 0))
