...
" pyxlscript indent file based on python
" Language:         pyxlscript
" Maintainer:       Stephan Steinbach
" Original Author:  Stephan Steinbach
" License:          CC0
"
" Based on the python indent file by Daniel Mahler.
"
...
setlocal nolisp
setlocal autoindent
setlocal indentexpr=GetpyxlscriptIndent(v:lnum)
setlocal indentkeys=!^F,o,O,<:>,0),0],0},=elif,=except

" TODO: check required patch for timeout argument, likely lower than 7.3.429 though.
if !exists('g:pyxlscript_indent_searchpair_timeout')
    if has('patch-8.0.1483')
        let g:pyxlscript_indent_searchpair_timeout = 150
    else
        let g:pyxlscript_indent_searchpair_timeout = 0
    endif
endif
...
function! s:find_opening_paren(lnum, col)
    ...
    let nearest = [0, 0]
    let timeout = g:pyxlscript_indent_searchpair_timeout
...
function! s:indent_like_opening_paren(lnum)
    ...
    let hang_closing = get(b:, 'pyxlscript_indent_hang_closing',
                \ get(g:, 'pyxlscript_indent_hang_closing', 0))
...
" Is the syntax at lnum (and optionally cnum) a pyxlscript string?
function! s:is_pyxlscript_string(lnum, ...)
    ...
    for cnum in cols
        if match(map(synstack(a:lnum, cnum),
                    \ "synIDattr(v:val, 'name')"), 'pyxlscript\S*String') == -1
            return 0
        end
    endfor
    return 1
...
function! GetpyxlscriptIndent(lnum)
    ...
    if s:is_pyxlscript_string(a:lnum-1, max([1, len(prevline)]))
                \ && (s:is_pyxlscript_string(a:lnum, 1)
                \     || match(line, '^\%("""\|''''''\)') != -1)
        ...
        if match_quotes != -1
            ...
            let pairpos = searchpairpos(quotes, '', quotes, 'bW', '', 0, g:pyxlscript_indent_searchpair_timeout)
            if pairpos[0] != 0
        ...
        if s:is_pyxlscript_string(a:lnum-1)
        ...
        let indent_multi = get(b:, 'pyxlscript_indent_multiline_string',
                    \ get(g:, 'pyxlscript_indent_multiline_string', 0))
